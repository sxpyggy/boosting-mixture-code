...
zero_NULL<-mean(dat_test$pi_null)+(1-mean(dat_test$pi_null))*exp(-mean(dat_test$lambda_null))
```
...
zero_glm_cp<-mean(dat_test$pi_glm)+(1-mean(dat_test$pi_glm))*exp(-mean(dat_test$lambda_glm))
```
...
exp(-mean(dat_test$y))
sum(dat_test$y==0)/length(dat_test$y)
zero_glm_vp<-mean(dat_test$pi_glm)+(1-mean(dat_test$pi_glm))*exp(-mean(dat_test$lambda_glm))
```
...
exp(-mean(dat_test$y))
sum(dat_test$y==0)/length(dat_test$y)
zero_bst_vp<-mean(dat_test$pi_boost)+(1-mean(dat_test$pi_boost))*exp(-mean(dat_test$lambda_boost))
```

# comparison

```{r}
loss_mat<-data.frame(model=c("null","glm_cp","glm_vp","bst_cp","bst_vp","true"),negL=c(loss_null,loss_glm_cp,loss_glm_vp,loss_BST_cp,loss_BST_vp,loss_true),error_eta=NA, error_pif=NA)
loss_mat$error_eta<-c(eNULL_eta, eGLM_cp_eta,eGLM_vp_eta,eBST_cp_eta,eBST_vp_eta,NA)
loss_mat$error_pif<-c(eNULL_pif,eGLM_cp_pif,eGLM_vp_pif,eBST_cp_pif,eBST_vp_pif,NA)
loss_mat$proportion_0<-c(zero_NULL,zero_glm_cp,zero_glm_vp,zero_bst_cp,zero_bst_vp,zero_true)
loss_mat
