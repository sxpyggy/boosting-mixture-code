...
Second, boosting algorithm is overfitting-sensitive, if we add suitable normalization to boosting step we can perform variable selection simultaneously. 
...
\section{Reviews: mixture of models and the EM algorithm}
...
\subsection{EM algorithm}
	...
	Suppose we know which component distribution each sample is from. That is we know the \textit{full information} $(Y,Z,\bx)$ where
...
Note that the multinomial logistic classification are fitted to all samples, while $K$ regressions are fitted to {partial} samples with $\{i:z_{i,k}=1\}$.

However, we do not have full information  $(Y,Z,\bx)$ but only the incomplete information $(Y,\bx)$. The following EM algorithm is inspired by the above discussion.
\paragraph{Expectation step.}
	With iterated $\hat{p},\hat{\mu},\hat{\phi}$, calculate the \textit{conditional expectation} of $z$:
...
\paragraph{Maximization step.}
	...
	\begin{itemize}
		\item A multinomial logistic classification with {fractional} response.
		\item $K$ {weighted}-regressions fitted to {all} samples.
	\end{itemize}
...
\subsection{Generic functional gradient descent algorithm}
	...
	Suppose {non-parametric regression function} as $F:\R^P\rightarrow\R$. Our purpose is to  estimate $F$ to minimize the expected loss $$\hat{F}=\underset{F}{\arg\min}\E\left[C(Y,F(\bx))\right],$$
...
We follow \citet{buehlmann:2003} to review the generic functional gradient decent algorithm:
	...
	\begin{enumerate}
		...
		Data $(u_i,\bx_i)_{i=1:n}$ is used to calibrate a weak learner $\hat{f}^{[m+1]}(\bx;\hat{\theta}^{[m+1]})$ with \textit{loss function $L_2$}.
		...
		where $s$ is \textit{shrinkage factor or learning rate}.
...
In the algorithm, weak learners are fitted to negative gradient $U$ rather than $Y$.
		 ...
		 If weak learners are trees, the algorithm is called \textit{gradient boosting decision tree (GBDT)}.
		 If weak learners are trees,  {calibration and variable selection} are performed simultaneously.
		\citet{buhlmann2007boosting} argue that step 3 of one-dimensional optimization seems unnecessary given learning rate is sufficiently small according to some empirical experiments.
...
We denote the regression functions for $p,\mu,\phi$ by {$F,G,H$}. In the EB algorithm, we need to specify link functions and cost/loss functions, and derive negative gradient function.
...
\begin{equation}
	\begin{aligned}
		{C_{Z}(Z, F(\bx))}=& - \sum_{k=1}^K Z_k \log p_k(\bx).
	\end{aligned}
\end{equation}
The negative gradient of the cost function w.r.t. $F_k$ is given by
\begin{equation}
	{U_k(Z,F(\bx))}=-\frac{\partial C_{Z}(Z,F(\bx))}{\partial F_k(\bx)}=
...
Similarly, we use negative log-likelihood function of each component model as its cost function:
	\begin{equation}
		\begin{aligned}
			{C_k(Y,Z,G(\bx))}=& -Z_k\log f_k(Y;\mu_k(G_k(\bx)),\phi_k), ~ k=1:K.
		\end{aligned}
	\end{equation}
	The negative gradient of $C_k$ w.r.t. $G_k$ is denoted by ${V_k(Y,Z,G(\bx))}$.
...
\begin{enumerate}
	...
	\item[3.2] Gradient boosting component models $\hat{\mu}^{[t+1]}$ given latent variable $\hat{z}^{[t]}$.
...
In step 3.2 of gradient boosting component models, we implement $K$ weighted boosting:
	\begin{enumerate}
		...
		\item[3.2.4]  Compute the updates
...
In step 4, for parameters not related to covariate $\bx$, compute the MLE $\hat{\phi}_k^{[t+1]}$ given all the other parameters $\hat{p}_k^{[t+1]}, \hat{\mu}_k^{[t+1]}$, then increase $t$ by 1 and repeat steps 2-3 until $t$
...
In the EB algorithm, we have the following tuning parameters and the corresponding tuning strategies are listed below:
	\begin{itemize}
		\item Number of EB iterations \blue{$T$}. This can be determined by screening the trace plot of loss.
		...
		\item Learning rate \blue{$s$}. Smaller learning rate tends to lead to a better fitting and predictive performance but requiring more iterations.
		\item Tuning parameters in base learner tree: complexity parameter, maximum depth, number of terminal nodes. Those parameters can be tuned following the routine in typical tree models.
	\end{itemize}

		
{\bf Remarks.}
	In initialization of boosting, we initialize parameters {independently} with the previously boosting estimates $\hat{p}_k^{[t-1]}, \hat{\mu}_k^{[t-1]}$. We call this as independent boosting.
	In contrast, we might initialize parameters as the previously boosting estimates $$\hat{p}_k^{[t,0]}=\hat{p}_k^{[t-1]}, \hat{\mu}_k^{[t,0]}=\hat{\mu}_k^{[t-1]}.$$
		This would lead a {smaller} required boosting iterations $M$ (or a {earlier} stop on validation loss). We call this as forward boosting. 
