...
It is well known that insurance loss data sometimes cannot be well modeled by a single distribution due to its complex data structures. For instance, Poisson distribution may not fit the claim count data well because of an excess of zero claims.
...
When the individual risk factors are available, a mixture of regressions is then a natural extension to modeling the risk heterogeneity, which is captured by additional features or risk factors.
In the literature,
...
In the field of machine learning research, a mixture model is often called a mixture of experts models, which is an ensemble learning technique that implements the idea of training experts on subtasks of a predictive modeling problem \citep{jacobs1991adaptive,jiang1999hierarchical}.
...
To overcome it,  \citet{dempster1977maximum} proposed the Expectation-Maximization (EM) algorithm, which is an iterative method to estimate the component parameters and the latent component indicator variables. Although the EM algorithm becomes a standard approach for estimation of mixture models, it is hard to address other practical issues, for example, the variable selection and the feature engineering, both of which are also challenging since the dependence between the covariates and response variables varies from one component to another.  
...
We propose an Expectation-Boosting (EB) algorithm for mixture models, which replaces the maximization step with a boosting step. 
...
Second, the boosting algorithm is overfitting-sensitive \citep{buehlmann:2003}. This property is beneficial since the ultimate goal of insurance loss modeling is to predict future losses. Therefore,
...
Section \ref{sec:application} studies two simulated data and one real data to illustrate the proposed methodology. Section \ref{sec:conclusions} concludes the paper with several important findings.  The R code of our work is publicly available on the github (\url{https://github.com/sxpyggy/boosting-mixture-code}).
...
Suppose a random variable $y$ follows a finite mixture of distributions with the $k$-th {\it component distribution} from a finite collection of distributions
	...
	\end{itemize}
	Selection among the above model alternatives can be viewed as imposing suitable constraints on the general form \eqref{mix-gen}, which accelerates and stabilizes model fitting without compromising predictive performance. It provides practical flexibility according to different data structures and goals of modeling.
...
\paragraph{Maximization step.}
	...
	\begin{equation}\label{likelihood}
		...
		\end{aligned}
	\end{equation}
 the MLE of regression coefficients is given by the following $K+1$ independent optimizations
...
To make it possible for non-parametric modeling of component models, we replace the maximization step in the EM algorithm with a boosting step.
The boosting algorithm estimates the regression functions non-parametrically and facilitates more flexible shapes.
...
In the GLM framework, the link function connects the linear predictor with the parameters of interest, such as mean and probability odds.
...
Notice that in our setting of statistical modeling, we first specify the likelihood function and the link function, then derive the cost function.
...
Before applying a boosting algorithm, we need to specify the likelihood function and the link function.
...
For mixing probabilities, we connect the mixing probabilities $p_1(\bx),\ldots,p_K(\bx)$ with the to-be-boosted regression functions $F_1(\bx),\ldots,F_K(\bx)$ by the multiple logistic link function $g_k:(0,1)^K\rightarrow \R$ such that 
...
Thus, those boosting steps can be conducted simultaneously to reduce the running time; see Table \ref{gaussian-summary}.
...
{\bf 1. Initialization of EB algorithm (line 1)}. We initialize the following quantities:
	\begin{itemize}
	...
	where $\mu^{-1}_k$ denotes the link function in the component model $k$.
		\item Initialize $\hat{\phi}_1^{[0]},\ldots, \hat{\phi}_K^{[0]}$ as the sample variance. 
...
{\bf 2. Expectation (line 3)}. We can calculate the conditional expectation of latent component indicator variable given  $\hat{p}^{[t-1]}_k(\bx),\hat{\mu}^{[t-1]}_k(\bx),\hat{\phi}^{[t-1]}_k$, $k=1,\ldots,K$:
	\begin{equation*}
	...
	\end{equation*}
Note that the conditional expectation carries the information obtained from the last EB iteration $t-1$.
Although the initialization lines 4 and 13 reset the boosting at each EB iteration, the conditional expectations $\hat{z}_{i,k}^{[t]}$ interact with the boosting via the negative gradients \eqref{p-gradient} and \eqref{comp-gradient} and the loss functions \eqref{p-loss} and \eqref{comp-loss}. Lines 4-12 are a typical $K$-class logistic gradient boosting. 
...
{\bf 4. Projection of gradient to weak learner (line 7)}.
		We can compute the negative gradient for each data point $i=1,\ldots,n$ as
...
{\bf 6. Return $\hat{p}_k^{[t,m]}$ according to \eqref{logistic}, $k=1,\ldots,K$ (line 6)}.
...
In lines 6-9, $K$ independent boostings are performed for $F_k, k=1,\ldots,K$. Hence, we can accelerate lines 6-9 by parallel computing.
...
Lines 13-21 implement $K$ independent gradient boosting for each component mean.

{\bf 7. Initialization of component means (line 13)}. We set $\hat{\mu}_k^{[t,0]}(\bx),\hat{G}_k^{[t,0]}(\bx)$, $k=1,\ldots,K$ as:
...
{\bf 10. Return $\hat{\mu}_k^{[t,m]}$ according to corresponding link functions (line 19)}. The component means are updated as
		$$\hat{\mu}_k^{[t,m]}(\bx_i)=\mu_k(\hat{G}_k^{[t,m]}(\bx_i)),$$
		where $\mu_k$ denotes the inverse link function in component model $k$.

In lines 14-20, $K$ independent weighted boostings with weights $\hat{z}_{i,k}^{[t]}$ are implemented, we can accelerate this loop  by parallel computing.
...
{\bf 11. Estimation of dispersion (line 22)}. 
...
The EB algorithms have four types of hyperparameters: the number of EB iterations $T$, numbers of boosting iterations $M_p$ and $M_\mu$, learning rates $s_p$ and $s_\mu$, and hyperparameters in weak learners. 
...
\end{equation}
If there is no noticeable improvement of the validation loss, the corresponding boosting can be early stopped before reaching $M_p$ or $M_\mu$.  
...
One reason is that the boosting of the component means stops early while the boosting of mixing probabilities takes more time to discover the heterogeneity; see Figures \ref{gaussian-trace} and \ref{gaussian-comp}. 
We accelerate the EB algorithm by applying parallel computing in the boosting of mixing probabilities, i.e., in lines 6-9 of Algorithm \ref{EB} we fit $K$ trees simultaneously.
