...
dat_BST<-function(K,n){
  ...
  dat_bst<-data.frame(BST=matrix(rep(1,3*K*n),ncol=3*K,nrow=n))
  bst_names<-NULL
  for (k in 1:K){
...
dat_Norm<-function(K,n){
  # put into a suitable format
  dat_bst<-data.frame(BST=matrix(rep(1,7*K*n),ncol=7*K,nrow=n))
  bst_names<-NULL
  for (k in 1:K){
