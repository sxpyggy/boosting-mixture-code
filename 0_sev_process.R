...
VehBrandOrd$x<-round(VehBrandOrd$x,0)
names(VehBrandOrd)<-c("brand","severity","order")

dat$VehBrandOrd<-NA
for (k in 1:nrow(VehBrandOrd)){
  dat$VehBrandOrd[dat$VehBrand==VehBrandOrd$brand[k]]<-VehBrandOrd$order[k]
}
...
RegionOrd$x<-round(RegionOrd$x,0)
names(RegionOrd)<-c("region","severity","order")
dat$RegionOrd<-NA
for (k in 1:nrow(RegionOrd)){
  dat$RegionOrd[dat$Region==RegionOrd$region[k]]<-RegionOrd$order[k]
}
aggregate(dat$Sev,by=list(dat$RegionOrd),mean)
str(dat)

write.csv(RegionOrd,"./plots/sev/region_ord.csv")
write.csv(VehBrandOrd,"./plots/sev/brand_ord.csv")
