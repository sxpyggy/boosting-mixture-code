...
plot(density(dat_low$Sev[dat_low$ind<5],width = 100))
...
```{r}
gamma_fit<-glm(Sev ~ Area + VehAgeGLM + DrivAgeGLM + BonusMalusGLM + VehBrand + Region, data=dat_train[dat_train$Sev<M,], family = Gamma(link="log"))
summary(gamma_fit)

plot(log(dat_test$Sev[dat_test$Sev<M]),log(predict(gamma_fit,newdata = dat_test[dat_test$Sev<M,],type="response")))

gamma_fit<-glm(Sev ~ Area + VehAgeGLM + DrivAgeGLM + BonusMalusGLM + VehBrand + Region, data=dat_train[dat_train$bucket==1,], family = Gamma(link="log"))
summary(gamma_fit)

plot(log(dat_test$Sev[dat_test$bucket==1]),log(predict(gamma_fit,newdata = dat_test[dat_test$bucket==1,],type="response")))
```
...
```{r em}
### z (bucket selected)
(K<-length(unique(dat_train$bucket)))

z_hat<-matrix(NA,nrow=nrow(dat_train),ncol=K)
for ( k in 1:K){
  z_hat[,k]<-ifelse(dat_train$bucket==k,1,0)
}
...
for (k in 1:(K-1)){
  coef_v<-gamma_mle(dat_train$Sev, z_hat[,k])
  par_mat$mu[k]<-coef_v$mu
  par_mat$shape[k]<-coef_v$shape
  par_mat$scale[k]<-coef_v$scale
  par_mat$rate[k]<-coef_v$rate
}
#write.csv(round(par_mat,4),"../plots/sev_int.csv")
aggregate(dat_train$Sev,by=list(dat_train$bucket),mean)

### pareto threshold and tail index
theta<-M
(alpha<-alpha_wmle(y=dat_train$Sev, z=z_hat[,K],theta=theta))
```
...
for (k in 1:(K-1)){
  ...
  loss_train[i]<-mean(-log(f_mix_cp(dat_train$Sev,p_hat,par_mat,alpha,theta)))
  loss_valid[i]<-mean(-log(f_mix_cp(dat_valid$Sev,p_hat,par_mat,alpha,theta)))
  print(c(i,loss_train[i]))
}

# png("../plots/sev/null_trace.png")
matplot(cbind(loss_train,loss_valid)[-c(1:20),],type="l",xlab="iterations of EM", ylab="neg L")
legend("topright",c("training loss","validation loss"),lty=c(1,2),col=c("black","red"))
# dev.off()
p_hat
par_mat
alpha
...
alpha_null<-alpha
#write.csv(round(par_mat_null,4),"../plots/sev_null.csv")
```
...
for(i in 1:J){
  ...
  # MLE of p (given Z)
  glm_p<- multinom(z_hat ~ Area + VehAgeGLM + DrivAgeGLM + BonusMalusGLM + VehBrand + Region, data=dat_train, trace = F)
...
for (k in 1:(K-1)){
  ...
  loss_train[i]<-mean(-log(f_mix_vp(dat_train$Sev,p_hat_mat,par_mat,alpha,theta)))
  loss_valid[i]<-mean(-log(f_mix_vp(dat_valid$Sev,p_hat_mat_valid, par_mat,alpha,theta)))
  print(c(i,loss_train[i]))
}

matplot(cbind(loss_train,loss_valid),type="l",col=c("red","gray"),lty=1)
apply(p_hat_mat,2,mean);p_hat
par_mat
# write.csv(round(par_mat,4),"../plots/sev_glm.csv")
alpha
...
for (i in 1:J) {
  ...
  boost_four <-
    blackboost(
      Sev ~ Area + VehAgeGLM + DrivAgeGLM + BonusMalusGLM + VehBrand + Region,
      weights = z_hat[, 4],
      data = dat_train,
      family = GammaReg(),
  ...
  dat_train$four <- fitted(boost_four, type = "response")
  dat_test$four <-
    predict(boost_four, newdata = dat_test, type = "response")
  
  logL_shape <- function(shape_mle) {
    sum(z_hat[, 4]) * shape_mle * log(shape_mle) -
      sum(z_hat[, 4] * log(dat_train$four)) * shape_mle -
      sum(z_hat[, 4]) * lgamma(shape_mle) +
      (shape_mle - 1) * sum(z_hat[, 4] * log(dat_train$Sev)) -
      shape_mle * sum(z_hat[, 4] * dat_train$Sev / dat_train$four)
  }
  ...
  f_mat[, K] <- dpareto(y = dat_train$Sev,
                        theta = theta,
                        alpha = alpha)
  ...
  loss_train[i] <- mean(-log(pf_sum))
  loss_test[i] <- mean(-log(pf_sum_test))
  print(c(i, loss_train[i]))
}

matplot(cbind(loss_train,loss_test),type="l",col=c("red","blue"),lty=1)
apply(p_hat_mat,2,mean);p_hat
par_mat
alpha
...
for (i in 1:J) {
  ...
  dat_train$four <- fitted(boost_four, type = "response")
  # dat_train$four <- fitted(glm_four, type = "response")
