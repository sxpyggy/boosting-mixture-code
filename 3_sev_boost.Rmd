...
plot(loss_learn,type="l",xlab="iterations of EM", ylab="neg L")
...
plot(loss_learn,type="l",col=c("red"),lty=1)
...
for(i in 1:J){
  ...
  # MLE of p (given Z)
  bst_p<- BST(X=dat_train[,variables_name], Y=z_hat_train, Pinit=NULL, Xval = dat_valid[,variables_name], Yval=z_hat_valid, Pvalinit = NULL, M=50, cp=0.001, maxdepth = 4, lr=0.1, trace = T, patience = 2)
...
for (i in 1:J) {
  # MLE of p (given Z)
  bst_p<- BST(X=dat_learn[,variables_name], Y=z_hat, Pinit=NULL, Xval = dat_learn[,variables_name], Yval=z_hat, Pvalinit = NULL, M=50, cp=0.001, maxdepth = 4, lr=0.1, trace = T, patience = 2)
...
test_loss<-data.frame(model=c("null","glm-p","bst-p","glm-p-mu","bst-p","bst-p-mu",loss=c(7.581538, 7.558955, 7.558401,7.578587, 7.557301)))

# write.csv(test_loss,"./plots/sev_loss.csv")
```
